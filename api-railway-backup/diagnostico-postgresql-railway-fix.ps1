    ...
    switch ($problemType) {
        "1" {
            ...
            if ($pgUser -and $pgPassword -and $pgPort -and $pgDatabase) {
                ...
                # Usar comillas simples para evitar interpretación de variables
                $connString = "postgresql://${pgUser}:${escapedPassword}@localhost:${pgPort}/${pgDatabase}"
                railway variables set DATABASE_URL="$connString"
            }
        ...
        "3" {
            ...
            if ($pgUser -and $pgHost -and $pgPort -and $pgDatabase) {
                Write-ColorMessage "Actualizando DATABASE_URL..." $Yellow
                $escapedPassword = $pgPasswordText
                # Usar notación URI para evitar problemas con caracteres especiales
                $escapedPassword = [Uri]::EscapeDataString($escapedPassword)
                $connString = "postgresql://${pgUser}:${escapedPassword}@${pgHost}:${pgPort}/${pgDatabase}"
                railway variables set DATABASE_URL="$connString"
            }
