...
// IMPORTANTE: Catch-all route para manejar cualquier endpoint de API que no exista
app.get('/api/stories/:id/*', (req, res) => {
  console.log(`⚠️ Endpoint específico no encontrado: ${req.path}, devolviendo respuesta vacía`);
  res.json({
    message: 'Endpoint específico no encontrado',
    storyId: parseInt(req.params.id),
    path: req.path,
    suggestion: 'Usa /api/stories/:id/likes o /api/stories/:id/comments'
  });
});

app.get('/api/*', (req, res) => {
  console.log(`⚠️ Endpoint general no encontrado: ${req.path}, devolviendo respuesta vacía`);
  res.json({
    message: 'Endpoint no encontrado',
    path: req.path,
    suggestion: 'Usa /api/test para probar la API o /api/test-db para verificar la conexión'
  });
});
